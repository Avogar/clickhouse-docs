...
## 4. Create your own database and table

As in most databases management systems, ClickHouse logically groups tables into **databases**. Use the `CREATE DATABASE` command to create a new database in ClickHouse:
  ```
  CREATE DATABASE IF NOT EXISTS helloworld
  ```

ClickHouse supports several **table engines**.  The engine determines:

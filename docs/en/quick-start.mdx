...
# Quick Start
 ...
 <font size="5">
ClickHouse runs on ClickHouse Cloud, or on any Linux, FreeBSD, or Mac OS X system with x86_64, AArch64, or PowerPC64LE CPU architecture. Follow these steps to get up and running with ClickHouse.
 </font> 
...
3. Run a query. For example, the following returns the names of the databases:
  ```
  SHOW databases
  ```
...
## 3. Create a database, and a table in that database
...
2. Even the simplest of tables in ClickHouse must specify a **table engine**. The engine determines details about the table like:
  ...
  There are many engines to choose from, but for a simple table on a single-node ClickHouse server, [MergeTree](/en/engines/table-engines/mergetree-family/mergetree.md) is your likely choice. Run the following command to create a table named `my_first_table` in the `helloworld` database:
  ```
